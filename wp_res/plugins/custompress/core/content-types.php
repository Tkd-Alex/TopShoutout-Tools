...
//Allow shortcodes in Widgets
	...
	class CustomPress_Content_Types extends CustomPress_Core {
		...
		function fields_shortcode( $atts, $content = null ) {
			...
			if($vvsdd == "company"){
				global $wpdb;
				$post_id1 = $wpdb->get_results("SELECT * FROM `wppt_woocommerce_order_itemmeta` WHERE (meta_key = '_product_id' AND `meta_value` = '". $bpsds ."') LIMIT 1");
				foreach($post_id1 as $postd) {
					$pids=$postd->order_item_id;
					$post_id2 = $wpdb->get_results("SELECT * FROM `wppt_woocommerce_order_items` WHERE (`order_item_id` = '$pids') LIMIT 1");
					foreach($post_id2 as $postd1) {
						$pidsB=$postd1->order_id;
						$post_id3 = $wpdb->get_results("SELECT * FROM `wppt_postmeta` WHERE (`meta_key` = '_customer_user' AND `post_id`='$pidsB') LIMIT 1");
						foreach($post_id3 as $postd2) {
							$pidsD=$postd2->meta_value;
						}
					}
				}
			}
			...
			foreach ( $custom_fields as $custom_field ) {
				...
				if ( $output ) {
					...
					if ( $value != '' ) {
						if( in_array($custom_field['field_id'], $customfileds) ) {
							if( ($current_user == $BSA && $vvsd == "influencer") || ($vvsd == "administrator") || ($vvsd == "company" && $current_user == $pidsD)) {
								$result .='<p style="margin-left: -20px;padding-bottom: 0;">'.$custom_field['field_title'].': <b>'.$value.'</b></p>';
								if($custom_field['field_id'] == "Instagram__text_846a" ) $result .='<p style="margin-left: -20px;padding-bottom: 0;" >Email : <b>'.$user_eem.'</b></p>';
							}
							...
							$pricingdtls .= ( $custom_field['field_title'] ).": ";
							$pricingdtls .= $fmt['close_title'];
							$pricingdtls .= $fmt['open_value'];

							$pricingdtls .= strpos($value, '$') !== false ? "<b>".$value."</b>" : "<b>$".$value."</b>";

							$pricingdtls .= $fmt['close_value'];
							$pricingdtls .= $fmt['close_line'];
						}
